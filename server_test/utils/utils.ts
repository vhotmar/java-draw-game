...
export async function expectMessage<
  T extends NonNullable<ServerMessage["message"]>
>(client: Client, type: T): Promise<NonNullable<ServerMessage[T]>> {
...
export async function sendConnectMessage(
  ...
  await client.send(
    ClientMessage.create({
      connectMessage: ClientMessage.ConnectMessage.create({ name, roomId }),
    })
  );
}

export async function sendStartGameMessage(client: Client) {
  await client.send(
    ClientMessage.create({ startGame: ClientMessage.StartGameMessage.create() })
  );
}
